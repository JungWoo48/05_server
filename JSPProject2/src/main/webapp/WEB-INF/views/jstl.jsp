...
<body>
	<h1>JSTL(JSP standard tag libary)</h1>
	<pre>
		JSP에서 자주 사용되거나 공통적으로 사용되는 Java코드를
		표기법을 간단히 태그화 하여 표준으로 제공하는 것
		
		(if, for, 배열/컬렉션 길이, 문자열치화느 숫자 데이터형식변경, 데이터파싱, scope변수 선언등)
	</pre>
    ...
    <pre>
    	...
    	(현재 태그 내부에 if~else ㄹ르 작성)
    	
    	c:when : if/ else if 를 나타내는 태그
    			속성은 test밖에 없ㅇ,ㅁ(조건 작성 속성)
    			
    	c:otherwise : sle를 나타내는 태그
    			아무런 속성이 없다		
    </pre>
